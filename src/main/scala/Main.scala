...
object Main extends App {
  private val statusDisplay = document.querySelector(".game--status")

  private val initialState = Game(active = true, "X", List("", "", "", "", "", "", "", "", ""))
  ...
  private def handlePlayerChange(game: Game) = {
    val game2 =
  ...
  private def isWin(game: Game)(winCondition: List[Int]) =
  ...
  private def handleRestartGame = UIO {
  ...
  implicit class ElementExt(element: Element) {
  ...
  def run(args: List[String]): URIO[ZEnv, ExitCode] = {
    ...
    def updateGame(game: Game) = UIO {
      _game = game
    }

    ZIO
      .foreach_(document.querySelectorAll(".cell")) {
